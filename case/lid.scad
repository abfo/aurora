...
// bottom of case, this needs to get to 75mm on y for the speakers and another 17mm on each side for the width of each speaker...
difference() {
    translate([-55.5,-37.5,0]){
        color([.8,.2,.2])
        cube(size=[131, 75, 3], center = false);
    }

    translate([-29,-24.5,0]){
        // lcd
        translate([22,25,0]){
            cube(size=[30, 30, 40], center = true);
        }
        // left mic
        translate([3.5, 18, -6]){
            cylinder(h=12,d=7);
...
translate([0,45,0]) {
    difference() {
        cylinder(h=23.5,d=5.5);
        cylinder(h=5,d=4.6);
        metric_thread(2.5, 1 ,30, internal=true);
    }
}
